...
get(int key)
{
  ...
  pthread_mutex_lock(&nreads_lock);
  inc(num_reads);
  if(num_reads == 1){
    pthread_mutex_lock(&locks[i]);       // acquire lock
  }
  pthread_mutex_unlock(&nreads_lock);
  ...
  dec(num_reads);
  if(num_reads == 0){
    pthread_mutex_unlock(&locks[i]);       // acquire lock
  }
  pthread_mutex_unlock(&nreads_lock);
...
main(int argc, char *argv[])
{
  ...
  pthread_mutex_init(&nreads_lock, NULL);
